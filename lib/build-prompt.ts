...
const truncateByTokens = (text: string, maxTokens: number, fromEnd = false) => {
...
const isUnifiedPromptVariant = (prompt: string) =>
  prompt.includes("你是用户“阿”的长期 AI 学术与项目顾问")
...
const buildBasePrompt = (
  ...
  fullPrompt += `Core System Instructions:\n${UNIFIED_SYSTEM_PROMPT}\n\n`
  fullPrompt +=
    "Formatting Policy:\nUse clean Markdown by default (headings/lists/tables/code). For math, use `$...$` (inline) and `$$...$$` (block).\n\n"
  ...
  if (memorySummaries.length > 0) {
    const memoryText = memorySummaries
      .map((item, index) => {
        const clippedSummary = truncateByTokens(item.summary || "", 220, true)
        return `${index + 1}. ${clippedSummary}`
      })
      .join("\n")
  ...
  if (
    ...
    prompt.trim() !== UNIFIED_SYSTEM_PROMPT.trim() &&
    !isUnifiedPromptVariant(prompt)
  ) {
...
export async function buildFinalMessages(
  ...
  const shouldIncludeWorkspaceInstructions =
    workspaceInstructions.trim().length > 0
      ? true
  ...
  const RESERVED_USER_MESSAGE_TOKENS = Math.min(
    1024,
    Math.max(384, Math.floor(CHUNK_SIZE * 0.28))
  )
  const MAX_PROMPT_TOKENS = Math.max(
    64,
    Math.min(
      Math.max(64, CHUNK_SIZE - 64),
      CHUNK_SIZE - RESERVED_USER_MESSAGE_TOKENS
    )
  )
  const promptForBudget = truncateByTokens(BUILT_PROMPT, MAX_PROMPT_TOKENS)
  const PROMPT_TOKENS = encode(promptForBudget).length
  ...
  if (finalMessages.length === 0 && processedChatMessages.length > 0) {
    const lastMessage = processedChatMessages[processedChatMessages.length - 1]
      .message
    const fallbackBudget = Math.max(96, CHUNK_SIZE - PROMPT_TOKENS)

    finalMessages.unshift({
      ...lastMessage,
      content: truncateByTokens(lastMessage.content, fallbackBudget, true)
    })
  }
